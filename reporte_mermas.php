<?php
// reporte_mermas.php - VERSIÓN VANGUARD PRO (FLUJO NATIVO + GRÁFICOS)
session_start();
...
try {
    ...
    $sql = "SELECT m.*, p.descripcion, p.precio_costo, u.usuario 
            FROM mermas m 
    ...
    $totalPerdida = 0;
    $total_mermas = count($registros);
    $mermas_producto = [];
    $mermas_motivo = [];

    foreach($registros as $r) {
        $perdida = floatval($r['cantidad']) * floatval($r['precio_costo']);
        $totalPerdida += $perdida;

        // Agrupar para Gráfico de Barras (Productos que más dinero pierden)
        $prod = strtoupper($r['descripcion']);
        if(!isset($mermas_producto[$prod])) { $mermas_producto[$prod] = 0; }
        $mermas_producto[$prod] += $perdida;

        // Agrupar para Gráfico Circular (Frecuencia de Motivos)
        $motivo = strtoupper($r['motivo'] ?? 'OTROS');
        if(!isset($mermas_motivo[$motivo])) { $mermas_motivo[$motivo] = 0; }
        $mermas_motivo[$motivo]++;
    }
    arsort($mermas_producto);
    arsort($mermas_motivo);
...
<head>
    <meta charset="UTF-8">
    <title>Reporte de Mermas - Vanguard Pro</title>
...
<body>
    ...
    <div id="reporteContenido" class="report-page">
        <table>
                    ...
                    <td colspan="6" style="border:none; padding:0; background:white;">
                        <header>
                            ...
                            <div style="text-align: right; width: 30%; font-size: 8pt; color: #333; font-weight: normal;">
                                <strong>REPORTE DE MERMAS</strong><br><?php echo $rango_texto; ?><br>
                        ...
                        <h3 style="color: #102A57; border-left: 5px solid #102A57; padding-left: 10px; margin-bottom: 20px; margin-top:0; text-transform: uppercase; font-size: 11pt;">
                            Detalle de Bajas de Inventario
                        </h3>
                    </td>
                </tr>
                <tr>
            ...
            <tbody>
                <?php if($total_mermas > 0): ?>
                    <?php foreach($registros as $r): 
                        $perdida_fila = $r['cantidad'] * $r['precio_costo'];
                    ?>
                    <tr class="evitar-corte">
                        ...
                        <td style="text-align: center; font-weight: bold;">-<?php echo floatval($r['cantidad']); ?></td>
                        <td style="text-align: right; font-weight: bold; color:#dc3545;">-$<?php echo number_format($perdida_fila, 2, ',', '.'); ?></td>
                    </tr>
                    <?php endforeach; ?>
                    
                    <tr class="evitar-corte">
                        <td colspan="6" style="text-align: center; padding: 40px 20px; color: #a0a0a0; border-top: 2px dashed #e0e0e0;">
                            <i class="bi bi-trash3" style="font-size: 28pt; display: block; margin-bottom: 10px; color: #d0d0d0;"></i>
                            <span style="font-size: 10pt; font-weight: bold; text-transform: uppercase; letter-spacing: 2px;">Fin de Registros de Mermas</span>
                        </td>
                    </tr>
                <?php else: ?>
                    <tr><td colspan="6" style="text-align:center; padding: 30px; color:#666;">No hubo mermas registradas en este periodo.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
        ...
        <div class="evitar-corte" style="margin-top: 30px;">
            <div class="resumen-card">
                <h4 style="color: #102A57; border-bottom: 2px solid #102A57; padding-bottom: 5px; margin-bottom: 15px; margin-top:0; text-transform: uppercase; font-size: 11pt;">Resumen Ejecutivo de Pérdidas</h4>
                ...
                <div style="display: flex; gap: 20px; margin-bottom: 20px;">
                    <div style="flex: 1; background: white; padding: 10px; border-radius: 5px; border: 1px solid #ddd; text-align: center;">
                        ...
                        <div style="font-size: 16pt; font-weight: 900; color: #102A57;"><?php echo $total_mermas; ?></div>
                    </div>
                    <div style="flex: 1; background: #fdf2f2; padding: 10px; border-radius: 5px; border: 1px solid #dc3545; text-align: center;">
                        <small style="color: #dc3545; font-weight: bold; text-transform: uppercase; font-size: 7pt;">Pérdida Valorizada Total</small>
                        <div style="font-size: 16pt; font-weight: 900; color: #dc3545;">-$<?php echo number_format($totalPerdida, 2, ',', '.'); ?></div>
                    </div>
                </div>
                ...
                <div style="display: flex; justify-content: space-between; gap: 15px; width: 100%; box-sizing: border-box;">
                    <div style="flex: 1; background: white; padding: 15px; border-radius: 5px; border: 1px solid #ddd; box-sizing: border-box;">
                        <strong style="font-size: 8pt; color: #666; text-transform: uppercase; display:block; text-align:center; margin-bottom:10px;">Top Productos con Pérdida ($)</strong>
                        <div style="position: relative; height: 220px; width: 100%;"><canvas id="chartProductos"></canvas></div>
                    </div>
                    <div style="flex: 1; background: white; padding: 15px; border-radius: 5px; border: 1px solid #ddd; box-sizing: border-box;">
                        <strong style="font-size: 8pt; color: #666; text-transform: uppercase; display:block; text-align:center; margin-bottom:10px;">Frecuencia de Motivos</strong>
                        <div style="position: relative; height: 220px; width: 100%;"><canvas id="chartMotivos"></canvas></div>
                    </div>
                </div>
            </div>
            
            <script> 
                const datosProductos = <?php echo json_encode($mermas_producto); ?>; 
                const datosMotivos = <?php echo json_encode($mermas_motivo); ?>; 
            </script>

            <div class="footer-section">
                <div style="width: 40%; font-size: 8pt; color: #666; line-height: 1.4; text-align: justify;">
                    <p><strong>DECLARACIÓN JURADA:</strong> Este reporte refleja fielmente las bajas de inventario y la pérdida de capital a precio de costo registradas en el sistema. Su validez puede ser verificada escaneando el código QR.</p>
                </div>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        // Gráfico de Barras: Productos que generan más pérdida
        if(typeof datosProductos !== 'undefined' && Object.keys(datosProductos).length > 0) {
            const allLabelsProd = Object.keys(datosProductos);
            const labelsProd = allLabelsProd.slice(0, 6).map(l => l.length > 13 ? l.substring(0, 13) + '...' : l);
            const dataProd = allLabelsProd.slice(0, 6).map(l => datosProductos[l]);
                ...
                new Chart(document.getElementById('chartProductos'), {
                    ...
                    data: { 
                        ...
                        datasets: [{ 
                            label: 'Pérdida ($)', 
                            data: dataProd, 
                            backgroundColor: '#dc3545', // Rojo para alerta de mermas
                            borderRadius: 4,
                            maxBarThickness: 35 
                        }] 
                    },
        ...
        // Gráfico Circular: Motivos de la Merma
        if(typeof datosMotivos !== 'undefined' && Object.keys(datosMotivos).length > 0) {
            ...
            const coloresMot = ['#dc3545', '#fd7e14', '#102A57', '#ffc107', '#6c757d', '#212529'];
    ...
    const optGlobal = { 
        margin: [15, 10, 25, 10], 
        filename: 'Reporte_Mermas_Corporativo.pdf', 
    ...
    function aplicarFormatoPaginas(pdf) {
        ...
        for (let i = 1; i <= totalPages; i++) {
            ...
            pdf.setDrawColor(220, 53, 69); // Línea Roja inferior como detalle estético para Mermas
            ...
            pdf.text('Reporte de Mermas - ' + textoEmpresa, 10, 284);
        ...
        Swal.fire({
            ...
            preConfirm: (email) => {
                ...
                return html2pdf().set(optGlobal).from(element).toPdf().get('pdf').then(function(pdf) {
                    ...
                    fData.append('email', email); fData.append('pdf_file', blob, 'Reporte_Mermas.pdf');
